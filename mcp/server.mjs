...
const TOOLS = [
  ...
  ['browser_open_tab', 'Open a tab with an optional URL in the focused Chrome window, or navigate an existing tab when tabId is provided.', { tabId: { type: 'number' }, windowId: { type: 'number' }, url: { type: 'string' }, active: { type: 'boolean' } }],
