...
Native Messaging 是本机 Chrome extension 和本机 native host 之间的 local transport。普通网页 JS 通常不能直接探测这条链路；本仓库仍只把它当作本地进程通信边界，不把它包装成网页可访问的 HTTP/WebSocket bridge。
...
默认 `tools/list` 只暴露只读 snapshot / query / wait / target-identification 能力：
...
这些工具不读取 cookies，不读取 Chrome 下载记录，不截图，不打开 / 切换 / reload tab。
...
调试工具不属于默认 runtime surface，不应用于上游日常自动化路径。
...
该脚本会执行 typecheck、build、manifest/tool/protocol/黑名单静态检查，以及 complex-layout、Boss-like mock、detectable-surface 三组本地运行时验收。静态检查会确认 manifest 只有 `activeTab`、`tabs`、`scripting`、`nativeMessaging` 四项权限，默认 `tools/list` 不含 cookies / downloads / screenshot / tab mutation / reload 工具，并检查源码没有 MAIN world、合成 MouseEvent / KeyboardEvent、DOM 写入或页面事件派发。
运行时验收需要打开 fixture tab，因此 smoke 脚本只在 L2 子步骤里临时启用 debug-only tab management。单独只跑 L1 可用：
...
- 自动把 `mcp/server.mjs` 注册到 `~/.codex/config.toml`
- 输出安装摘要和校验结果
...
- `native-host/host-launcher`

这个 launcher 会把当前机器上的 Node 绝对路径写进去，再启动 `native-host/host.mjs`。
这样 Chrome 启动 Native Messaging Host 时不依赖 `nvm`、shell profile 或 `PATH`，换电脑后只要重新执行 `npm run setup:auto` 即可。

如果你刚完成一次新的 `dist/` 构建，需要让 Chrome 载入最新扩展代码。稳定做法是先在 `chrome://extensions` 确认当前 unpacked extension 指向本仓库 `dist/`，手动点一次“重新加载”或重启 Chrome，再运行 acceptance。`browser_reload_extension` 仅在 `MCP_BROWSER_CHROME_DEBUG_TOOLS=1` 时作为单步调试工具暴露；默认 MCP runtime 不列出它。
...
3. 最近是否把新 `dist/` 载入当前扩展实例（优先在 `chrome://extensions` 手动 reload；`browser_reload_extension` 只在 debug-only tools 中用于单步调试）
...
- 上传 / 下载控件会额外带 `type` / `accept` / `multiple` / `href` / `download` 等语义字段，便于上游只读判断页面上是否存在文件相关入口；本仓库默认 runtime 不读取 Chrome 下载记录，也不定位本地下载 artifact
- 不产生页面可观察的合成交互事件
...
- 本仓库不处理 human-like behavior、人类行为模拟、键鼠节奏或输入策略；这些不属于 mcp-browser-chrome 的职责范围
