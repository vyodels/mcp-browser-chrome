...
>
> 2026-04-25 状态说明：本文是历史跨系统设计文档。当前 `mcp-browser-chrome` runtime 只保留只读 snapshot / URL / host / signature / viewport 等原始观察字段；`ActionContext`、VirtualHID 执行合同和 HID 学习归因以 `cross-project-runtime-docs` 与 `VirtualHID` 仓库当前文档为准，不属于 browser runtime 实现职责。
...
- `snapshot.viewport.screenX` / `screenY` — 浏览器可观察窗口指标；不要直接当权威 HID 内容视口原点使用
- `snapshot.viewport.outerWidth/Height` 与 `innerWidth/Height` — 非权威窗口/视口观察指标，可作为诊断输入，但 browser-mcp 不输出绝对 HID 映射结果
...
```
centerScreenX / centerScreenY 由 VirtualHID/OS 层基于窗口、内容区域和显示器坐标系统归一化得到；
browser-mcp 只提供页面语义、viewport/document clickPoint、tab/window id、URL/host 与下载记录证据。
```

pinch-zoom 场景（`visualViewport.scale !== 1`）的坐标归一化也放在 VirtualHID/OS 粘合层，browser-mcp 不成为绝对屏幕坐标权威。
...
其他方面：
- 这是 2026-04-23 历史基线：当时 browser-mcp 的 15 个只读工具不增不删；2026-04-25 起新增的 `browser_locate_download` 只读下载记录 / 本地路径定位属于 artifact 证据补口，不改变 browser/HID 动作边界
